...
def check(lib_dir):
    ...
    print("Number of items to check: ", number_of_rows, "\n")
    bar = progressbar.ProgressBar(maxval=number_of_rows)
    
    # Processing each item of DB
    with open('%s.log' % args.output_file, 'w') as log_file:    # Open Log File
        for masteritem in bar(iter(dbcursor.fetchone, None)):
            ...
            if not os.path.exists(path):
                if args.verbose:
                    print("Check Master", ntpath.normpath(masteritem['imagePath']), CRED, " NOK ", CEND)
                print("Master :\t UUID=", uuid, "\tArquivo: ", path, file=log_file)
            else:
                if args.verbose:
                    print("Check Master ", ntpath.normpath(masteritem['imagePath']), CGREEN, " OK ", CEND)
            ...
            continue       # FOR NOW, not to check the versions
                ...
                for version in iter(version_cursor.fetchone, None):
                    ...
                    if version['adjustmentUuid'] != 'UNADJUSTEDNONRAW':
                        ...
                        adjust_cursor.execute('SELECT * FROM RKModelResource WHERE resourceTag=?', [version['adjustmentUuid']])
                        for resource in iter(adjust_cursor.fetchone, None):
                            if resource['attachedModelType'] == 2 and resource['resourceType'] == 4:
                                ...
                                if res_filename != None and not os.path.exists(full_file):
                                    if args.verbose:
                                        print("     Check Version : ", full_file, CRED, " NOK ", CEND)
                                    print("Versão :\t UUID=", version_uuid, "\tArquivo: ", version['filename'], file=log_file)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ev', action="store_true", default=True, help='Exclude check of Verion Files (Edited Photos). Scan only Master Files') 
    ...
    parser.add_argument('output_file', help='name of Log the results (no extension)')
    args = parser.parse_args()

    print("Checking Library: ", args.library_dir)
    check(args.library_dir)
